...
避免工具调用和系统事件产生伪分支。第三个“会话子项目”视图可以把两类对话归入用户维护的
虚拟子项目，分类只保存在扩展工作区状态中，不移动或修改原始对话文件。
...
4. 右键对话选择“归入子项目…”，在“会话子项目”视图中跨 provider 分类管理。
5. 右键子项目可重命名或删除；删除分类只会把对话移回“未分类”。
6. 点击图中的节点查看提问、回复、思考、工具调用和工具结果。
7. 用顶部“分支”下拉框只查看 `HEAD` 或某个 `tip`，同时保留其完整祖先链。
