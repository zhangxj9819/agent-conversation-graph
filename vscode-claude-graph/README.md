...
- **虚拟子项目**：在第三个统一视图中把 Claude/Codex 对话归类管理，不移动原始记录。
...
app-server 的 `thread/fork` 接口创建。子项目及对话归类只保存在 VS Code 的工作区状态中；
创建、重命名或删除子项目都不会改写或移动 Claude/Codex JSONL。
...
4. 在对话条目的右键菜单中选择“归入子项目…”，或在“会话子项目”标题栏中新建分类。
5. 在“会话子项目”中查看跨 Claude/Codex 的归类；右键子项目可以重命名或删除分类。
6. 点击图节点查看该轮的提问、回复、思考、工具调用和工具结果。
7. 用顶部“分支”下拉框只查看 `HEAD` 或某个 `tip`；图会保留该分支到根节点的完整祖先链。
8. 选中历史节点创建新分支，或从 `HEAD` / `tip` 继续已有分支。
9. 在会话条目上点击垃圾桶，确认后将整个对话谱系移到系统废纸篓。

删除子项目只会把其中的对话移回“未分类”。需要移动对话时再次选择“归入子项目…”，
需要解除归类时选择“移出子项目”。这些操作均不触碰原始会话文件。
